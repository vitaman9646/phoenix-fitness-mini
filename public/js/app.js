...
// ===============================
const CONFIG = {
  // ВАЖНО: Не храни токены в клиентском коде!
  // Используй serverless функцию (Netlify Functions, Vercel, Cloudflare Workers)
  API_ENDPOINT: '/api/submit-form', // Endpoint для отправки формы
...
class Quiz {
  ...
  getRecommendedPlan(goal, frequency, level) {
    ...
    if (level === 'beginner' || frequency <= 3) {
      return {
        name: 'Старт',
        description: 'Попробуй мой подход без долгих обязательств. План на неделю, рекомендации по питанию, чат-поддержка.',
        price: '990₽/неделя'
      };
    } else if (level === 'advanced' || frequency >= 5) {
      return {
        name: 'Премиум',
        description: 'Для тех, кто готов выложиться на 100%. Видео-звонки, разбор техники в реальном времени, психологическая поддержка.',
        price: '59 990₽ за 12 недель'
      };
    } else {
      return {
        name: 'Стандарт',
        description: 'Оптимальный вариант для большинства. Полная программа на 8 недель с контролем техники и корректировками.',
        price: '39 990₽ за 8 недель'
      };
    }
  }
  
  downloadBonus() {
    // Здесь можно генерировать PDF или отправлять на email
    Utils.showNotification('Отлично!', 'Бонусы отправлены на твой email (если указал) или скачай по ссылке', 'success');
    Utils.trackEvent('Quiz', 'DownloadBonus', '');
...
class CTAForm {
  ...
  async sendForm(data) {
    ...
    // ВАРИАНТ 2: Прямая отправка в Telegram (НЕ рекомендуется, токен виден)
    // См. комментарий в следующей секции
  }
}
